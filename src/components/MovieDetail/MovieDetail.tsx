...
const MovieDetail: React.FC<IMovieDetail> = ({
  ...
  return (
    <div className="bg-indigo-950 text-white p-8 md:p-24 rounded-lg shadow-lg mx-auto">
      <div className="flex flex-col md:flex-row items-center gap-10">
        <img src={IMAGE_SOURCE + poster_path} alt="Poster de la película" className="w-80 md:w-96 rounded" />
        <div>
          <h3 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 italic">{original_title}</h3>
          <p className="text-lg mb-6 text-gray-300">{overview}</p>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-lg mb-6">
            <span className="flex items-center">
              <svg className="w-6 h-6 text-white" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
              ...
              <p className="pl-1 text-sm">18+</p>
            </span>
            <span className="flex items-center">
              <svg className="w-6 h-6 text-white" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                <path fillRule="evenodd" d="M2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10S2 17.523 2 12Zm11-4a1 1 0 1 0-2 0v4a1 1 0 0 0 .293.707l3 3a1 1 0 0 0 1.414-1.414L13 11.586V8Z" clipRule="evenodd"/>
              </svg>
              <p className="pl-1 text-sm">{runtime} min.</p>
            </span>
            <span className="flex items-center">
              <svg className="w-6 h-6 text-white" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
              ...
              <p className="pl-1 text-sm">{release_date}</p>
            </span>
            <span className="flex items-center">
              <svg className="w-6 h-6 text-white" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                <path d="M13.849 4.22c-.684-1.626-3.014-1.626-3.698 0L8.397 8.387l-4.552.361c-1.775.14-2.495 2.331-1.142 3.477l3.468 2.937-1.06 4.392c-.413 1.713 1.472 3.067 2.992 2.149L12 19.35l3.897 2.354c1.52.918 3.405-.436 2.992-2.15l-1.06-4.39 3.468-2.938c1.353-1.146.633-3.336-1.142-3.477l-4.552-.36-1.754-4.17Z"/>
              </svg>
              <p className="pl-1 text-sm">{vote_average}</p>
            </span>
            <span className="flex items-center">
              <svg className="w-6 h-6 text-white" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
              ...
              <p className="pl-1 text-sm">{vote_count}</p>
            </span>
          </div>
